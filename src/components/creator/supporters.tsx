...
export function RecentSupporters({ donations }: { donations: PublicDonation[] }) {
  ...
  return (
    <section>
      <h2 className="label-mono mb-4">Recent supporters</h2>
      <div className="space-y-3">
        {donations.slice(0, 6).map((donation) => {
          ...
          return (
            <div key={donation.id} className="flex items-center gap-3 rounded-lg border border-neutral-800/60 bg-neutral-950/40 p-3">
              ...
              <span className="shrink-0 font-mono text-sm text-white">{formatAmount(donation.amount, donation.currency)}</span>
            </div>
          );
        })}
      </div>
